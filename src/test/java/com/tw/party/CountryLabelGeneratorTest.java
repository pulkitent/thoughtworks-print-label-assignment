...
class CountryLabelGeneratorTest {
    @Test
    @DisplayName("Should generate only 2 labels i.e. Default and Reverse for guests from India only")
        //Positive Test
    void generateLabels() {
        String countryIndia = "India";
        List<Guest> guests = this.createTestDataWithThreeGuest();
        LabelGenerator labelGenerator = new CountryLabelGenerator(countryIndia);
        Integer expectedNumberOfLabels = 2;
    ...
    @Test
    @DisplayName("Should not generate labels for guests from USA")
